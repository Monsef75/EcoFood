...
<script>
    ...
    export default {
        ...
        data() { return {
            boxes: [
                { icon: 'fa-users', title: 'All Users', color: '#1877F2', data: null,},
                { icon: 'fa-arrow-down-wide-short', title: 'Joined This Month', color: '#FE2C55', data: null,},
                { icon: 'fa-arrow-down-wide-short', title: 'Joined This Week', color: '#fbad50', data: null,},                
            ],
            THead: [
                "#","Photo","User Name","Email","Porducts","Joined In","Process"
            ],
            Users: [],
        ...
        created() {
            this.setUsers().then( res => {
                if ( res.usersNbr == 0 ) this.empty = true
                else {
                    res.users.forEach( User => { User.waiting = false })                    
                    this.Users = res.users
                    this.boxes.forEach( box => box.data = res.usersNbr )
                }
                this.loading = false
            })
        },
